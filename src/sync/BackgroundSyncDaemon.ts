...
class BackgroundSyncDaemonClass {
  ...
  private getBridge(): BackgroundSyncBridge | null {
    const bridge = NativeModules.BackgroundSyncBridge as
    ...
    try {
      ...
      if (bridge?.canRunBackgroundSync) {
        ...
        if (!canRun) {
          MobileTelemetryService.trackBackgroundSyncStat(
            'background_tick_skipped',
            { reason: 'battery_or_power_policy' },
            'info',
          );
          return;
        }
  ...
  async start(
    intervalMs: number = DEFAULT_BACKGROUND_INTERVAL_MS,
  ): Promise<void> {
    if (this.running) {
      return;
    }
    ...
    Logger.info(
      TAG,
      `Background sync daemon started (interval=${intervalMs}ms, platform=${Platform.OS})`,
    );
    MobileTelemetryService.trackBackgroundSyncStat(
