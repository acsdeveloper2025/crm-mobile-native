...
import { WatermarkReStampQueue, type ReStampJob } from '../../services/WatermarkReStampQueue';
...
export const WatermarkReStamper: React.FC = () => {
  ...
  const [currentJob, setCurrentJob] = useState<(ReStampJob & { address: string }) | null>(null);
  ...
  const processNext = useCallback(async () => {
    ...
    try {
      ...
      const photoToUse = (await RNFS.exists(job.rawPhotoPath)) ? job.rawPhotoPath : job.savedPhotoPath;
      ...
      const preciseLocation = await new Promise<{ lat: number; lng: number; alt: number; spd: number; accuracy: number; heading?: number } | null>(resolve => {
      ...
      const address = await LocationService.getAddressFromCoordinates(bestLocation.lat, bestLocation.lng);
      if (!address) {
        Logger.warn(TAG, `Address fetch failed for ${job.attachmentId}, skipping`);
        WatermarkReStampQueue.dequeue();
        processingRef.current = false;
        return;
      }
      ...
      WatermarkReStampQueue.dequeue();
      setCurrentJob({ ...job, rawPhotoPath: photoToUse, location: { ...job.location, ...bestLocation }, address });

      // Wait for React to render the ViewShot
      await new Promise(resolve => setTimeout(resolve, 600));

      // Capture the rendered watermark
      const uri = await captureRef(viewShotRef, { format: 'jpg', quality: 0.85 });
      ...
      if (job.rawPhotoPath !== job.savedPhotoPath && await RNFS.exists(job.rawPhotoPath)) {
  ...
  return (
    <View style={styles.hidden}>
      <ViewShot ref={viewShotRef} style={styles.viewShot} options={{ format: 'jpg', quality: 0.85 }}>
        <Image source={{ uri: `file://${currentJob.rawPhotoPath}` }} style={StyleSheet.absoluteFillObject} resizeMode="cover" />
        ...
        <View style={styles.watermarkStrip}>
          <View style={styles.gpsCard}>
            ...
            <Text style={styles.gpsDecimal}>{location.lat.toFixed(6)}, {location.lng.toFixed(6)}</Text>
...
const styles = StyleSheet.create({
  hidden: { position: 'absolute', left: 0, top: 0, width: 720, height: 1280, opacity: 0, pointerEvents: 'none' as const, zIndex: -1 },
  viewShot: { width: 720, height: 1280, backgroundColor: 'black' },
  watermarkStrip: { position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', backgroundColor: 'rgba(0,0,0,0.72)', paddingVertical: 10, paddingHorizontal: 12, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: 'rgba(255,255,255,0.15)' },
  gpsCard: { width: 110, marginRight: 10, paddingRight: 10, borderRightWidth: StyleSheet.hairlineWidth, borderRightColor: 'rgba(255,255,255,0.2)' },
  gpsIconRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
  gpsLabel: { color: '#22d3ee', fontSize: 8, fontWeight: '800', letterSpacing: 1.2, marginLeft: 4 },
  gpsCoordDMS: { color: '#ffffff', fontSize: 10, fontWeight: '700', lineHeight: 14 },
  gpsDecimal: { color: '#94a3b8', fontSize: 8, fontWeight: '500', marginTop: 2 },
  gpsAccuracy: { color: '#4ade80', fontSize: 8, fontWeight: '600', marginTop: 1 },
  dataStack: { flex: 1, justifyContent: 'center' },
  dataRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 2 },
  dataLabel: { color: '#cbd5e1', fontSize: 10, fontWeight: '500', marginLeft: 4 },
  addressText: { color: '#fde68a', fontSize: 9, fontWeight: '600', marginLeft: 4, flexShrink: 1, lineHeight: 13 },
  brandRow: { flexDirection: 'row', alignItems: 'center', marginTop: 3 },
  brandText: { color: '#64748b', fontSize: 7, fontWeight: '700', letterSpacing: 0.8, textTransform: 'uppercase' },
  brandDot: { width: 3, height: 3, borderRadius: 1.5, backgroundColor: '#64748b', marginHorizontal: 5 },
});
