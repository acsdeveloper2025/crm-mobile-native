...
import {
  VerificationTask, PositivePropertyApfReportData, BuildingStatusApf, FlatStatusApf, FinalStatus, TaskStatus,
  AddressLocatable, AddressRating, RelationshipApf
} from '../../../types';
...
const PositivePropertyApfForm: React.FC<PositivePropertyApfFormProps> = ({ taskData }) => {
  const navigation = useNavigation<any>();
  const { theme } = useTheme();
  const { updatePositivePropertyApfReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  
  const report = taskData.positivePropertyApfReport;
  ...
  const isFormValid = useMemo(() => {
    ...
    const baseFields: (keyof PositivePropertyApfReportData)[] = [
        ...
        'metPerson', 'relationship', 'otherObservation', 'finalStatus'
    ];
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updatePositivePropertyApfReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updatePositivePropertyApfReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updatePositivePropertyApfReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No Positive Property APF report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.PROPERTY_APF_POSITIVE}
      formData={report}
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Positive Property APF Report</Text>
