...
import {
  VerificationTask, ShiftedPropertyApfReportData, PropertyStatus, FinalStatus, TaskStatus,
...
interface ShiftedPropertyApfFormProps {
  taskData: VerificationTask;
}

const ShiftedPropertyApfForm: React.FC<ShiftedPropertyApfFormProps> = ({ taskData }) => {
  const navigation = useNavigation<any>();
  const { theme } = useTheme();
  const { updateShiftedPropertyApfReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  
  const report = taskData.shiftedPropertyApfReport;
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateShiftedPropertyApfReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateShiftedPropertyApfReport(taskData.id, data.formData);
    }
  };
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof ShiftedPropertyApfReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof ShiftedPropertyApfReportData)[] = [
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateShiftedPropertyApfReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateShiftedPropertyApfReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateShiftedPropertyApfReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No Shifted Property APF report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={'property-apf-shifted' as any}
      formData={report}
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Shifted Property APF Report</Text>
...
export default ShiftedPropertyApfForm;
