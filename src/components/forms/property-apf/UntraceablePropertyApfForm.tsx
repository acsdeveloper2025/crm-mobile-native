...
import {
  VerificationTask, UntraceablePropertyApfReportData, FinalStatus, TaskStatus,
  LocalityTypeResiCumOffice, DominatedArea, CallRemarkUntraceable
} from '../../../types';
...
const UntraceablePropertyApfForm: React.FC<UntraceablePropertyApfFormProps> = ({ taskData }) => {
  ...
  const { updateUntraceablePropertyApfReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateUntraceablePropertyApfReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateUntraceablePropertyApfReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateUntraceablePropertyApfReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateUntraceablePropertyApfReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateUntraceablePropertyApfReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No Untraceable Property APF report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Untraceable Property APF Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <FormField label="Contact Person" id="metPerson" name="metPerson" value={report.metPerson} onChangeText={(val) => handleChange('metPerson', val)} disabled={isReadOnly} />
