...
import {
  VerificationTask, NspPropertyApfReportData, AddressLocatable, AddressRating,
  PropertyStatus, FinalStatus, TaskStatus
} from '../../../types';
...
const NspPropertyApfForm: React.FC<NspPropertyApfFormProps> = ({ taskData }) => {
  const navigation = useNavigation<any>();
  const { theme } = useTheme();
  const { updateNspPropertyApfReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  
  const report = taskData.nspPropertyApfReport;
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateNspPropertyApfReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateNspPropertyApfReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateNspPropertyApfReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No NSP Property APF report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.PROPERTY_APF_NSP}
      formData={report}
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>NSP Property APF Report</Text>
