...
const PositiveNocForm: React.FC<PositiveNocFormProps> = ({ taskData }) => {
  ...
  const { updatePositiveNocReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updatePositiveNocReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updatePositiveNocReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updatePositiveNocReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updatePositiveNocReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updatePositiveNocReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[styles.errorText, styles.noDataText]}>No Positive NOC report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Positive NOC Report</Text>
        ...
        {report.officeStatus === OfficeStatusOffice.Opened && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>NOC Verification Details</Text>
            ...
            <FormField label="Authorised Signature" id="authorisedSignature" name="authorisedSignature" value={report.authorisedSignature} onChangeText={(val) => handleChange('authorisedSignature', val)} disabled={isReadOnly} />
            <FormField label="Name on NOC" id="nameOnNoc" name="nameOnNoc" value={report.nameOnNoc} onChangeText={(val) => handleChange('nameOnNoc', val)} disabled={isReadOnly} />
            <FormField label="Flat / Shop No." id="flatNo" name="flatNo" value={report.flatNo} onChangeText={(val) => handleChange('flatNo', val)} disabled={isReadOnly} />
          </View>
        )}
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <TextAreaField label="Other Remarks" id="otherExtraRemark" name="otherExtraRemark" value={report.otherExtraRemark} onChangeText={(val) => handleChange('otherExtraRemark', val)} disabled={isReadOnly} />
        </View>
