...
import {
  ...
  TPCConfirmation, LocalityTypeResiCumOffice, DominatedArea, FeedbackFromNeighbour, FinalStatus, TaskStatus
} from '../../../types';
...
const NspNocForm: React.FC<NspNocFormProps> = ({ taskData }) => {
  ...
  const { updateNspNocReport, toggleSaveTask , fetchTasks , updateTaskStatus} = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateNspNocReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateNspNocReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateNspNocReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateNspNocReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateNspNocReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[styles.errorText, styles.noDataText]}>No NSP NOC report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>NSP NOC Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <SelectField label="Business Existence" id="businessExistance" name="businessExistance" value={report.businessExistance || ''} onValueChange={(val) => handleChange('businessExistance', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
            {options.businessExistence}
          </SelectField>
          <SelectField label="Applicant Existence" id="applicantExistance" name="applicantExistance" value={report.applicantExistance || ''} onValueChange={(val) => handleChange('applicantExistance', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
