...
import {
  VerificationTask, UntraceableNocReportData, CallRemarkUntraceable, LocalityTypeResiCumOffice, DominatedArea, FinalStatus, TaskStatus
} from '../../../types';
...
const UntraceableNocForm: React.FC<UntraceableNocFormProps> = ({ taskData }) => {
  ...
  const { updateUntraceableNocReport, toggleSaveTask , fetchTasks , updateTaskStatus} = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateUntraceableNocReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateUntraceableNocReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateUntraceableNocReport(taskData.id, { [name]: value });
  };

  const handleImagesChange = (images: any[]) => {
    updateUntraceableNocReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateUntraceableNocReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[styles.errorText, styles.noDataText]}>No Untraceable NOC report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Untraceable NOC Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <View style={styles.grid}>
            ...
            <View style={styles.gridItemFull}>
              <Text style={styles.infoLabel}>Visit Address</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <FormField label="Contact Person" id="contactPerson" name="contactPerson" value={report.contactPerson} onChangeText={(val) => handleChange('contactPerson', val)} disabled={isReadOnly} />
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <TextAreaField label="Other Extra Remark" id="otherExtraRemark" name="otherExtraRemark" value={report.otherExtraRemark} onChangeText={(val) => handleChange('otherExtraRemark', val)} disabled={isReadOnly} />
        </View>
