...
const ShiftedNocForm: React.FC<ShiftedNocFormProps> = ({ taskData }) => {
  ...
  const { updateShiftedNocReport, toggleSaveTask , fetchTasks , updateTaskStatus} = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateShiftedNocReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateShiftedNocReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateShiftedNocReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateShiftedNocReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateShiftedNocReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[styles.errorText, styles.noDataText]}>No Shifted NOC report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Shifted NOC Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <View style={styles.grid}>
            ...
            <View style={styles.gridItem}>
              ...
              <Text style={[styles.infoValue, { color: theme.colors.text }]}>{taskData.notes || taskData.trigger || 'N/A'}</Text>
            </View>
            ...
            <View style={styles.gridItem}>
              <Text style={styles.infoLabel}>System Contact Number</Text>
              <Text style={[styles.infoValue, { color: theme.colors.text }]}>{taskData.systemContactNumber || 'N/A'}</Text>
            </View>
            <View style={styles.gridItem}>
              <Text style={styles.infoLabel}>Applicant Status</Text>
              <Text style={[styles.infoValue, { color: theme.colors.text }]}>{taskData.applicantStatus || 'N/A'}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <FormField label="Current Company Period" id="currentCompanyPeriod" name="currentCompanyPeriod" value={report.currentCompanyPeriod} onChangeText={(val) => handleChange('currentCompanyPeriod', val)} placeholder="e.g., 3 years" disabled={isReadOnly} />
        ...
        {report.officeStatus === OfficeStatusOffice.Opened && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Office Details</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Property & Locality assessment</Text>
