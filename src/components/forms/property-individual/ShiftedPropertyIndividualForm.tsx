...
import {
  VerificationTask, ShiftedPropertyIndividualReportData, PropertyStatus, FinalStatus, TaskStatus,
...
interface ShiftedPropertyIndividualFormProps {
  taskData: VerificationTask;
}

const ShiftedPropertyIndividualForm: React.FC<ShiftedPropertyIndividualFormProps> = ({ taskData }) => {
  const navigation = useNavigation<any>();
  const { theme } = useTheme();
  const { updateShiftedPropertyIndividualReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks() as any;
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  
  const report = taskData.shiftedPropertyIndividualReport;
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateShiftedPropertyIndividualReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateShiftedPropertyIndividualReport(taskData.id, data.formData);
    }
  };
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof ShiftedPropertyIndividualReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof ShiftedPropertyIndividualReportData)[] = [
        'addressLocatable', 'addressRating', 'propertyStatus',
        'personMet', 'designation', 'otherObservation', 'finalStatus'
    ];
    
    return checkFields(baseFields);
  }, [report, MIN_IMAGES]);

  const handleChange = (name: string, value: any) => {
    if (isReadOnly) return;
    updateShiftedPropertyIndividualReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateShiftedPropertyIndividualReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateShiftedPropertyIndividualReport(taskData.id, { selfieImages });
  ...
  const options = useMemo(() => ({
    ...
    propertyStatus: getEnumOptions(PropertyStatus),
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No Shifted Property Individual report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={'property-individual-shifted' as any}
      formData={report}
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Shifted Property Individual Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Property Verification</Text>
          ...
          <SelectField label="Property Status" id="propertyStatus" name="propertyStatus" value={report.propertyStatus || ''} onValueChange={(val) => handleChange('propertyStatus', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
            {options.propertyStatus}
          </SelectField>
          <FormField label="Person Met" id="personMet" name="personMet" value={report.personMet} onChangeText={(val) => handleChange('personMet', val)} disabled={isReadOnly} />
          <FormField label="Designation" id="designation" name="designation" value={report.designation} onChangeText={(val) => handleChange('designation', val)} disabled={isReadOnly} />
...
export default ShiftedPropertyIndividualForm;
