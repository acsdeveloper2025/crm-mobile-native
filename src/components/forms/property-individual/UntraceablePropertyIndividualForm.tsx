...
import {
  VerificationTask, UntraceablePropertyIndividualReportData, FinalStatus, TaskStatus,
  CallRemarkUntraceable, LocalityTypeResiCumOffice, DominatedArea
} from '../../../types';
...
const UntraceablePropertyIndividualForm: React.FC<UntraceablePropertyIndividualFormProps> = ({ taskData }) => {
  ...
  const { updateUntraceablePropertyIndividualReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateUntraceablePropertyIndividualReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateUntraceablePropertyIndividualReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateUntraceablePropertyIndividualReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateUntraceablePropertyIndividualReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateUntraceablePropertyIndividualReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No Untraceable Property Individual report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Untraceable Property Individual Report</Text>
