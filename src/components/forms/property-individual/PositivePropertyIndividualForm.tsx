...
import {
  VerificationTask, PositivePropertyIndividualReportData, FinalStatus, TaskStatus,
  AddressLocatable, AddressRating, BuildingStatusApf, FlatStatusApf, RelationshipApf,
...
const PositivePropertyIndividualForm: React.FC<PositivePropertyIndividualFormProps> = ({ taskData }) => {
  ...
  const { updatePositivePropertyIndividualReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updatePositivePropertyIndividualReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updatePositivePropertyIndividualReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updatePositivePropertyIndividualReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updatePositivePropertyIndividualReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updatePositivePropertyIndividualReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No Positive Property Individual report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Positive Property Individual Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <View style={styles.grid}>
            ...
            <View style={styles.gridItem}>
                <SelectField label="Flat Status" id="flatStatus" name="flatStatus" value={report.flatStatus || ''} onValueChange={(val) => handleChange('flatStatus', val)} disabled={isReadOnly}>
                    <Picker.Item label="Select..." value="" />
                    {options.flatStatus}
                </SelectField>
            </View>
        ...
        {report.flatStatus === FlatStatusApf.Opened && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Additional Details (Flat Open)</Text>
            ...
            <SelectField label="Relationship" id="relationship" name="relationship" value={report.relationship || ''} onValueChange={(val) => handleChange('relationship', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
              {options.relationship}
            </SelectField>
            <FormField label="Approx Area (Sq. Feet)" id="approxArea" name="approxArea" value={report.approxArea?.toString()} onChangeText={(val) => handleChange('approxArea', val)} disabled={isReadOnly} />
          </View>
        )}
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Property Specifics</Text>
          ...
          <View style={styles.grid}>
            <View style={styles.gridItem}>
                <FormField label="Address Structure" id="addressStructure" name="addressStructure" value={report.addressStructure?.toString()} onChangeText={(val) => handleChange('addressStructure', val)} disabled={isReadOnly} />
            </View>
            <View style={styles.gridItem}>
                <FormField label="Address Exist At (Floor)" id="addressExistAt" name="addressExistAt" value={report.addressExistAt?.toString()} onChangeText={(val) => handleChange('addressExistAt', val)} disabled={isReadOnly} />
            </View>
          </View>
          ...
          <SelectField label="Society Name Plate" id="societyNamePlateStatus" name="societyNamePlateStatus" value={report.societyNamePlateStatus || ''} onValueChange={(val) => handleChange('societyNamePlateStatus', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
            {options.sightStatus}
          </SelectField>
