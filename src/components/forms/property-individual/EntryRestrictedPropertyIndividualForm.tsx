...
import {
  VerificationTask, EntryRestrictedPropertyIndividualReportData, FinalStatus, TaskStatus,
  AddressLocatable, AddressRating, FlatStatusApf, MetPersonErt, TPCConfirmation,
  LocalityTypeResiCumOffice, SightStatus, BuildingStatusApf, PoliticalConnection,
  DominatedArea, FeedbackFromNeighbour
} from '../../../types';
...
const EntryRestrictedPropertyIndividualForm: React.FC<EntryRestrictedPropertyIndividualFormProps> = ({ taskData }) => {
  ...
  const { updateEntryRestrictedPropertyIndividualReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateEntryRestrictedPropertyIndividualReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateEntryRestrictedPropertyIndividualReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateEntryRestrictedPropertyIndividualReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateEntryRestrictedPropertyIndividualReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateEntryRestrictedPropertyIndividualReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No Entry Restricted Property Individual report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Entry Restricted Property Individual Report</Text>
