...
import {
  VerificationTask, NspPropertyIndividualReportData, FinalStatus, TaskStatus,
...
const NspPropertyIndividualForm: React.FC<NspPropertyIndividualFormProps> = ({ taskData }) => {
  ...
  const { updateNspPropertyIndividualReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateNspPropertyIndividualReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateNspPropertyIndividualReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateNspPropertyIndividualReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateNspPropertyIndividualReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateNspPropertyIndividualReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={styles.noDataText}>No NSP Property Individual report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>NSP Property Individual Report</Text>
