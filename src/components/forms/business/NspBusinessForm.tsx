...
import {
  VerificationTask, NspBusinessReportData, OfficeStatusOffice, ApplicantExistence, SightStatus,
  TPCMetPerson, TPCConfirmation, LocalityTypeResiCumOffice, DominatedArea, FeedbackFromNeighbour,
  FinalStatus, TaskStatus, AddressLocatable, AddressRating, BusinessExistence, DesignationShiftedOffice,
  PremisesStatusBusiness
} from '../../../types';
...
const NspBusinessForm: React.FC<NspBusinessFormProps> = ({ taskData }) => {
  ...
  const { updateNspBusinessReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateNspBusinessReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateNspBusinessReport(taskData.id, data.formData);
    }
  };
  ...
  const isFormValid = useMemo(() => {
    ...
    const baseFields: (keyof NspBusinessReportData)[] = [
        ...
        'doorColor', 'landmark1', 'landmark2', 'dominatedArea', 'feedbackFromNeighbour',
    ...
    if (report.officeStatus === OfficeStatusOffice.Opened) {
        const openedOnlyFields: (keyof NspBusinessReportData)[] = [
            'applicantExistance', 'metPerson', 'designation', 'premisesStatus'
        ];
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateNspBusinessReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateNspBusinessReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateNspBusinessReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No NSP Business report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>NSP Business Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <View style={styles.grid}>
            <View style={styles.gridItemFull}>
              <SelectField label="Business Existance" id="businessExistance" name="businessExistance" value={report.businessExistance || ''} onValueChange={(val) => handleChange('businessExistance', val)} disabled={isReadOnly}>
                <Picker.Item label="Select..." value="" />
                {options.businessExistence}
              </SelectField>
            </View>
            <View style={styles.gridItemFull}>
              <SelectField label="Applicant Existance" id="applicantExistance" name="applicantExistance" value={report.applicantExistance || ''} onValueChange={(val) => handleChange('applicantExistance', val)} disabled={isReadOnly}>
                <Picker.Item label="Select..." value="" />
