...
import {
  VerificationTask, ShiftedBusinessReportData, OfficeStatusOffice, AddressLocatable, AddressRating,
  LocalityTypeResiCumOffice, DominatedArea, FeedbackFromNeighbour, FinalStatus, TaskStatus,
  PoliticalConnection, PremisesStatusBusiness, SightStatus, TPCMetPerson, TPCConfirmation, DesignationShiftedOffice
} from '../../../types';
...
const ShiftedBusinessForm: React.FC<ShiftedBusinessFormProps> = ({ taskData }) => {
  ...
  const { updateShiftedBusinessReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateShiftedBusinessReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateShiftedBusinessReport(taskData.id, data.formData);
    }
  };
  ...
  const isFormValid = useMemo(() => {
    ...
    const baseFields: (keyof ShiftedBusinessReportData)[] = [
        ...
        'otherObservation', 'finalStatus', 'oldOfficeShiftedPeriod'
    ];
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateShiftedBusinessReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateShiftedBusinessReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateShiftedBusinessReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Shifted Business report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Shifted Business Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <FormField label="Old Office Shifted Period (e.g., 6 months ago)" id="oldOfficeShiftedPeriod" name="oldOfficeShiftedPeriod" value={report.oldOfficeShiftedPeriod} onChangeText={(val) => handleChange('oldOfficeShiftedPeriod', val)} disabled={isReadOnly} />
        </View>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <View style={styles.subSection}>
            ...
            <SelectField label="TPC Met Person" id="tpcMetPerson" name="tpcMetPerson" value={report.tpcMetPerson || ''} onValueChange={(val) => handleChange('tpcMetPerson', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
          ...
          <View style={[styles.section, styles.highlightedSection]}>
            ...
            {report.premisesStatus !== PremisesStatusBusiness.Vacant && (
              <>
                ...
                <FormField label="Current Company Period (e.g., 2 years)" id="currentCompanyPeriod" name="currentCompanyPeriod" value={report.currentCompanyPeriod} onChangeText={(val) => handleChange('currentCompanyPeriod', val)} disabled={isReadOnly} />
              </>
            )}

            <FormField label="Approx Area (Sq. Feet)" id="approxArea" name="approxArea" value={report.approxArea || ''} onChangeText={(val) => handleChange('approxArea', val === '' ? null : Number(val))} type="number" disabled={isReadOnly} />
