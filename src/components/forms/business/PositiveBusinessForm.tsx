...
import {
  VerificationTask, PositiveBusinessReportData, OfficeStatusOffice, DesignationOffice,
  ...
  AddressLocatable, AddressRating, BusinessType, OwnershipTypeBusiness, AddressStatusBusiness
} from '../../../types';
...
const PositiveBusinessForm: React.FC<PositiveBusinessFormProps> = ({ taskData }) => {
  ...
  const { updatePositiveBusinessReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updatePositiveBusinessReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updatePositiveBusinessReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updatePositiveBusinessReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updatePositiveBusinessReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updatePositiveBusinessReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Positive Business report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Positive Business Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <View style={styles.grid}>
            ...
            <View style={styles.gridItemFull}>
              <FormField label="Business Period (e.g., 5 years)" id="businessPeriod" name="businessPeriod" value={report.businessPeriod} onChangeText={(val) => handleChange('businessPeriod', val)} disabled={isReadOnly} />
            </View>
          </View>
          ...
          <FormField label="Document Shown (e.g., Gumasta, Rent Deed)" id="documentShown" name="documentShown" value={report.documentShown} onChangeText={(val) => handleChange('documentShown', val)} disabled={isReadOnly} />
        </View>
        ...
        {report.officeStatus === OfficeStatusOffice.Opened && (
          <View style={[styles.section, styles.highlightedSection]}>
            ...
            <FormField label="Office Approx Area (Sq. Feet)" id="officeApproxArea" name="officeApproxArea" value={report.officeApproxArea || ''} onChangeText={(val) => handleChange('officeApproxArea', val === '' ? null : Number(val))} type="number" disabled={isReadOnly} />
            <FormField label="Staff Strength" id="staffStrength" name="staffStrength" value={report.staffStrength || ''} onChangeText={(val) => handleChange('staffStrength', val === '' ? null : Number(val))} type="number" disabled={isReadOnly} />
            <FormField label="Staff Seen" id="staffSeen" name="staffSeen" value={report.staffSeen || ''} onChangeText={(val) => handleChange('staffSeen', val === '' ? null : Number(val))} type="number" disabled={isReadOnly} />
          </View>
        )}
