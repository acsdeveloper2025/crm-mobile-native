...
import {
  VerificationTask, PositiveOfficeReportData, OfficeStatusOffice, DesignationOffice,
...
const PositiveOfficeForm: React.FC<PositiveOfficeFormProps> = ({ taskData }) => {
  ...
  const { updatePositiveOfficeReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updatePositiveOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updatePositiveOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updatePositiveOfficeReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updatePositiveOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updatePositiveOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Positive Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Positive Office Report</Text>
        ...
        {report.officeStatus === OfficeStatusOffice.Opened && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Work / Employment Details</Text>
            ...
            <FormField label="Working Period" id="workingPeriod" name="workingPeriod" value={report.workingPeriod} onChangeText={(val) => handleChange('workingPeriod', val)} disabled={isReadOnly} />
            <FormField label="Applicant Designation" id="applicantDesignation" name="applicantDesignation" value={report.applicantDesignation} onChangeText={(val) => handleChange('applicantDesignation', val)} disabled={isReadOnly} />
            ...
            <SelectField label="Working Premises" id="applicantWorkingPremises" name="applicantWorkingPremises" value={report.applicantWorkingPremises || ''} onValueChange={(val) => handleChange('applicantWorkingPremises', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
              {options.applicantWorkingPremises}
            </SelectField>
          {report.applicantWorkingPremises === ApplicantWorkingPremisesOffice.DifferentLocation && (
            <FormField label="Sitting Location" id="sittingLocation" name="sittingLocation" value={report.sittingLocation} onChangeText={(val) => handleChange('sittingLocation', val)} disabled={isReadOnly} />
          )}

          <SelectField label="Office Type" id="officeType" name="officeType" value={report.officeType || ''} onValueChange={(val) => handleChange('officeType', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
              {options.officeType}
            </SelectField>
            ...
            <FormField label="Staff Strength" id="staffStrength" name="staffStrength" value={report.staffStrength || ''} onChangeText={(val) => handleChange('staffStrength', val === '' ? null : Number(val))} disabled={isReadOnly} />
            <FormField label="Staff Seen" id="staffSeen" name="staffSeen" value={report.staffSeen || ''} onChangeText={(val) => handleChange('staffSeen', val === '' ? null : Number(val))} disabled={isReadOnly} />
            <FormField label="Office Approx Area (Sq. Feet)" id="officeApproxArea" name="officeApproxArea" value={report.officeApproxArea || ''} onChangeText={(val) => handleChange('officeApproxArea', val === '' ? null : Number(val))} disabled={isReadOnly} />
            <FormField label="Document Shown" id="documentShown" name="documentShown" value={report.documentShown} onChangeText={(val) => handleChange('documentShown', val)} disabled={isReadOnly} />
          </View>
        )}

        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Office Environment & Locality</Text>
          <FormField label="Establishment Period" id="establishmentPeriod" name="establishmentPeriod" value={report.establishmentPeriod} onChangeText={(val) => handleChange('establishmentPeriod', val)} disabled={isReadOnly} />
          <SelectField label="Name Plate Status" id="companyNamePlateStatus" name="companyNamePlateStatus" value={report.companyNamePlateStatus || ''} onValueChange={(val) => handleChange('companyNamePlateStatus', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
            {options.sightStatus}
          </SelectField>
          ...
          <View style={styles.subSection}>
            ...
            <SelectField label="TPC Met Person" id="tpcMetPerson1" name="tpcMetPerson1" value={report.tpcMetPerson1 || ''} onValueChange={(val) => handleChange('tpcMetPerson1', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
              {options.tpcMetPerson}
            </SelectField>
            <FormField label="Name of TPC" id="nameOfTpc1" name="nameOfTpc1" value={report.nameOfTpc1} onChangeText={(val) => handleChange('nameOfTpc1', val)} disabled={isReadOnly} />
            <SelectField label="TPC Confirmation" id="tpcConfirmation1" name="tpcConfirmation1" value={report.tpcConfirmation1 || ''} onValueChange={(val) => handleChange('tpcConfirmation1', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
          ...
          <View style={styles.subSection}>
            ...
            <SelectField label="TPC Met Person" id="tpcMetPerson2" name="tpcMetPerson2" value={report.tpcMetPerson2 || ''} onValueChange={(val) => handleChange('tpcMetPerson2', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
              {options.tpcMetPerson}
            </SelectField>
            <FormField label="Name of TPC" id="nameOfTpc2" name="nameOfTpc2" value={report.nameOfTpc2} onChangeText={(val) => handleChange('nameOfTpc2', val)} disabled={isReadOnly} />
            <SelectField label="TPC Confirmation" id="tpcConfirmation2" name="tpcConfirmation2" value={report.tpcConfirmation2 || ''} onValueChange={(val) => handleChange('tpcConfirmation2', val)} disabled={isReadOnly}>
              <Picker.Item label="Select..." value="" />
              {options.tpcConfirmation}
            </SelectField>
          </View>
