...
import {
  VerificationTask, EntryRestrictedOfficeReportData, OfficeStatusOffice, AddressLocatable, AddressRating, MetPersonErt, TPCConfirmation,
...
const EntryRestrictedOfficeForm: React.FC<EntryRestrictedOfficeFormProps> = ({ taskData }) => {
  ...
  const { updateEntryRestrictedOfficeReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateEntryRestrictedOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateEntryRestrictedOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const isFormValid = useMemo(() => {
    ...
    const baseFields: (keyof EntryRestrictedOfficeReportData)[] = [
        'addressLocatable', 'addressRating', 'metPerson', 'nameOfMetPerson', 'metPersonConfirmation', 'officeStatus', 'locality', 'addressStructure', 'officeExistFloor', 'addressStructureColor',
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateEntryRestrictedOfficeReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateEntryRestrictedOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateEntryRestrictedOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Entry Restricted Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Entry Restricted Office Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <NumberDropdownField label="Office Exist Floor" id="officeExistFloor" name="officeExistFloor" value={report.officeExistFloor || ''} onChange={(val: any) => handleChange('officeExistFloor', val)} min={1} max={300} disabled={isReadOnly} />
