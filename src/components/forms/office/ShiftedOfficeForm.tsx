...
import {
  VerificationTask, ShiftedOfficeReportData, OfficeStatusOffice, AddressLocatable, AddressRating,
  LocalityTypeResiCumOffice, DominatedArea, FeedbackFromNeighbour, FinalStatus, TaskStatus,
  DesignationShiftedOffice, SightStatus, TPCMetPerson, TPCConfirmation, PoliticalConnection
} from '../../../types';
...
const ShiftedOfficeForm: React.FC<ShiftedOfficeFormProps> = ({ taskData }) => {
  ...
  const { updateShiftedOfficeReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateShiftedOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateShiftedOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateShiftedOfficeReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateShiftedOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateShiftedOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Shifted Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Shifted Office Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <FormField label="Current Company Period" id="currentCompanyPeriod" name="currentCompanyPeriod" value={report.currentCompanyPeriod} onChangeText={(val) => handleChange('currentCompanyPeriod', val)} disabled={isReadOnly} />
          <FormField label="Old Office Shifted Period" id="oldOfficeShiftedPeriod" name="oldOfficeShiftedPeriod" value={report.oldOfficeShiftedPeriod} onChangeText={(val) => handleChange('oldOfficeShiftedPeriod', val)} disabled={isReadOnly} />
