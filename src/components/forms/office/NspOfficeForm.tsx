...
import {
  VerificationTask, NspOfficeReportData, OfficeStatusOffice, ApplicantExistence, SightStatus, OfficeExistence, DesignationShiftedOffice,
  TPCMetPerson, TPCConfirmation, LocalityTypeResiCumOffice, DominatedArea, FeedbackFromNeighbour,
  FinalStatus, TaskStatus, AddressLocatable, AddressRating
} from '../../../types';
...
const NspOfficeForm: React.FC<NspOfficeFormProps> = ({ taskData }) => {
  ...
  const { updateNspOfficeReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateNspOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateNspOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateNspOfficeReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateNspOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateNspOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No NSP Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>NSP Office Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Company Details</Text>

          {/* Office Existence - Always visible */}
          <SelectField label="Office Existance" id="officeExistence" name="officeExistence" value={report.officeExistence || ''} onValueChange={(val) => handleChange('officeExistence', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
