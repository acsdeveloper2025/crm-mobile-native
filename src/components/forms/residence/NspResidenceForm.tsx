...
const NspResidenceForm: React.FC<NspResidenceFormProps> = ({ taskData }) => {
  ...
  const { updateNspResidenceReport, toggleSaveTask, fetchTasks , updateTaskStatus} = useTasks();
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateNspResidenceReport(taskData.id, { [name]: processedValue });
  };
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No NSP Residence report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>NSP Residence Report</Text>
        ...
        {report.houseStatus === HouseStatus.Opened && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            ...
            <FormField label="Staying Period" id="stayingPeriod" name="stayingPeriod" value={report.stayingPeriod} onChangeText={(val) => handleChange('stayingPeriod', val)} placeholder="e.g., 6 months" disabled={isReadOnly} />
          </View>
        )}
        ...
        {report.houseStatus === HouseStatus.Closed && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Staying Person Details</Text>
