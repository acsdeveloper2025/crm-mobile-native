...
const PositiveBuilderForm: React.FC<PositiveBuilderFormProps> = ({ taskData }) => {
  ...
  const { updatePositiveBuilderReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updatePositiveBuilderReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updatePositiveBuilderReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updatePositiveBuilderReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updatePositiveBuilderReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updatePositiveBuilderReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Positive Builder report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Positive Builder Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <FormField label="Document Shown" id="documentShown" name="documentShown" value={report.documentShown} onChangeText={(val) => handleChange('documentShown', val)} placeholder="e.g., Gumasta, Rent Deed" disabled={isReadOnly} />
        </View>
        ...
        {report.officeStatus === OfficeStatusOffice.Opened && (
          <View style={[styles.section, styles.highlightedSection]}>
            ...
            <NumberDropdownField label="Office Approx Area (Sq. Feet)" id="officeApproxArea" name="officeApproxArea" value={report.officeApproxArea || ''} onChange={(val: any) => handleChange('officeApproxArea', val)} min={1} max={100000} disabled={isReadOnly} />
            <NumberDropdownField label="Staff Strength" id="staffStrength" name="staffStrength" value={report.staffStrength || ''} onChange={(val: any) => handleChange('staffStrength', val)} min={0} max={1000} disabled={isReadOnly} />
            <NumberDropdownField label="Staff Seen" id="staffSeen" name="staffSeen" value={report.staffSeen || ''} onChange={(val: any) => handleChange('staffSeen', val)} min={0} max={1000} disabled={isReadOnly} />
          </View>
        )}
