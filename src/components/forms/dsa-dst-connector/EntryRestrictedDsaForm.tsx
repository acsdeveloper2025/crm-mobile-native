...
import {
  ...
  OfficeStatusErtDsa, LocalityTypeResiCumOffice, PoliticalConnection, DominatedArea, FeedbackFromNeighbour, FinalStatus, TaskStatus
} from '../../../types';
...
const EntryRestrictedDsaForm: React.FC<EntryRestrictedDsaFormProps> = ({ taskData }) => {
  const navigation = useNavigation<any>();
    const { updateEntryRestrictedDsaReport, toggleSaveTask , fetchTasks , updateTaskStatus} = useTasks();
  ...
  if (!report) {
    return <Text>No Entry Restricted DSA/DST report data available.</Text>;
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <View>
      <Text>Entry Restricted DSA Report</Text>
      ...
      <View>
        ...
        <View>
          ...
          <NumberDropdownField label="Applicant Staying Floor" id="applicantStayingFloor" name="applicantStayingFloor" value={report.applicantStayingFloor || ''} onChange={(val) => handleChange('applicantStayingFloor', val)} min={1} max={300} disabled={isReadOnly} />
