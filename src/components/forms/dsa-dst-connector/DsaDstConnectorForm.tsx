...
import {
  VerificationTask, ConnectorStatus, FinalStatus, TaskStatus,
...
interface DsaDstConnectorFormProps {
  taskData: VerificationTask;
}

const DsaDstConnectorForm: React.FC<DsaDstConnectorFormProps> = ({ taskData }) => {
  const navigation = useNavigation<any>();
  const { theme } = useTheme();
  const { updateDsaDstConnectorReport, toggleSaveTask , fetchTasks, updateTaskStatus } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  
  const report = taskData.dsaDstConnectorReport;
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateDsaDstConnectorReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateDsaDstConnectorReport(taskData.id, data.formData);
    }
  };
    ...
    const checkFields = (fields: string[]) => fields.every(field => {
        const value = reportAny[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields = [
        'addressLocatable', 'addressRating', 'connectorStatus',
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateDsaDstConnectorReport(taskData.id, { [name]: value });
  };
  
  const handleImagesChange = (images: any[]) => {
    updateDsaDstConnectorReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: any[]) => {
    updateDsaDstConnectorReport(taskData.id, { selfieImages });
  ...
  const options = useMemo(() => ({
    ...
    connectorStatus: getEnumOptions(ConnectorStatus),
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[styles.errorText, styles.noDataText]}>No DSA/DST Connector report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={'dsa-dst-connector' as any}
      formData={report}
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>DSA/DST Connector Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Connector Verification</Text>
          ...
          {/* connectorStatus and personMet don't exist on PositiveDsaReportData */}
        </View>
...
export default DsaDstConnectorForm;
