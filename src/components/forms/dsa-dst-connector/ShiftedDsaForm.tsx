...
const ShiftedDsaForm: React.FC<ShiftedDsaFormProps> = ({ taskData }) => {
  const navigation = useNavigation<any>();
    const { updateShiftedDsaReport, toggleSaveTask , fetchTasks , updateTaskStatus} = useTasks();
  ...
  const handleChange = (name: string, value: any) => {
    ...
    // Check if the expected type for this specific field is a number based on existing logic
    if (name === 'approxArea') {
  ...
  if (!report) {
    return <Text>No Shifted DSA/DST report data available.</Text>;
  }
      ...
      {report.officeStatus === OfficeStatusOffice.Opened && (
        <View>
          ...
          <View>
            ...
            <FormField label="Approx Area (Sq. Feet)" id="approxArea" name="approxArea" value={report.approxArea || ''} onChangeText={(val) => handleChange('approxArea', val)} type="number" disabled={isReadOnly} />
          </View>
