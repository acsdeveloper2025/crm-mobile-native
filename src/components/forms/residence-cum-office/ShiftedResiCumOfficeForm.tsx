...
const ShiftedResiCumOfficeForm: React.FC<ShiftedResiCumOfficeFormProps> = ({ taskData }) => {
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateShiftedResiCumOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateShiftedResiCumOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateShiftedResiCumOfficeReport(taskData.id, { [name]: processedValue });
  };
  
  const handleImagesChange = (images: CapturedImage[]) => {
    updateShiftedResiCumOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: CapturedImage[]) => {
    updateShiftedResiCumOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Shifted Resi-cum-Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Shifted Residence-cum-Office Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <NumberDropdownField label="Floor" id="addressFloor" name="addressFloor" value={report.addressFloor || ''} onChange={(val) => handleChange('addressFloor', val)} min={1} max={300} disabled={isReadOnly} />
        ...
        {/* Assessment Section */}
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Assessment</Text>
