...
const UntraceableResiCumOfficeForm: React.FC<UntraceableResiCumOfficeFormProps> = ({ taskData }) => {
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateUntraceableResiCumOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateUntraceableResiCumOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateUntraceableResiCumOfficeReport(taskData.id, { [name]: processedValue });
  };
  
  const handleImagesChange = (images: CapturedImage[]) => {
    updateUntraceableResiCumOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: CapturedImage[]) => {
    updateUntraceableResiCumOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Untraceable Resi-cum-Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Untraceable Residence-cum-Office Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Investigation Details</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Location Details</Text>
