...
const NspResiCumOfficeForm: React.FC<NspResiCumOfficeFormProps> = ({ taskData }) => {
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateNspResiCumOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateNspResiCumOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateNspResiCumOfficeReport(taskData.id, { [name]: processedValue });
  };

  const handleImagesChange = (images: CapturedImage[]) => {
    updateNspResiCumOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: CapturedImage[]) => {
    updateNspResiCumOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No NSP Resi-cum-Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>NSP Residence-cum-Office Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <SelectField label="Address Traceable" id="addressTraceable" name="addressTraceable" value={report.addressTraceable || ''} onValueChange={(val) => handleChange('addressTraceable', val)} disabled={isReadOnly}>
          ...
          <SelectField label="Office Status" id="resiCumOfficeStatus" name="resiCumOfficeStatus" value={report.resiCumOfficeStatus || ''} onValueChange={(val) => handleChange('resiCumOfficeStatus', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
        ...
        {/* Conditional Details - Only show if office is open */}
        {report.resiCumOfficeStatus === ResiCumOfficeStatus.Open && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            ...
            <FormField label="Staying Period" id="stayingPeriod" name="stayingPeriod" value={report.stayingPeriod} onChangeText={(val) => handleChange('stayingPeriod', val)} placeholder="e.g., 2 years" disabled={isReadOnly} />
          </View>
        )}

        {/* Conditional Details - Only show if office is closed */}
        {report.resiCumOfficeStatus === ResiCumOfficeStatus.Closed && (
          <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.primary }]}>Confirmation Details (Office Closed)</Text>
            <FormField label="Staying Person Name" id="stayingPersonName" name="stayingPersonName" value={report.stayingPersonName} onChangeText={(val) => handleChange('stayingPersonName', val)} disabled={isReadOnly} />
          </View>
        )}
