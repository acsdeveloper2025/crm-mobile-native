...
const EntryRestrictedResiCumOfficeForm: React.FC<EntryRestrictedResiCumOfficeFormProps> = ({ taskData }) => {
  ...
  const handleFormDataChange = (formData: any) => {
    if (!isReadOnly) {
      updateEntryRestrictedResiCumOfficeReport(taskData.id, formData);
    }
  };
  ...
  const handleDataRestored = (data: any) => {
    if (!isReadOnly && data.formData) {
      updateEntryRestrictedResiCumOfficeReport(taskData.id, data.formData);
    }
  };
  ...
  const handleChange = (name: string, value: any) => {
    ...
    updateEntryRestrictedResiCumOfficeReport(taskData.id, { [name]: processedValue });
  };

  const handleImagesChange = (images: CapturedImage[]) => {
    updateEntryRestrictedResiCumOfficeReport(taskData.id, { images });
    handleAutoSaveImagesChange(images);
  };

  const handleSelfieImagesChange = (selfieImages: CapturedImage[]) => {
    updateEntryRestrictedResiCumOfficeReport(taskData.id, { selfieImages });
  ...
  if (!report) {
    return (
      <View style={styles.container}>
        <Text style={[{ color: theme.colors.danger }, styles.noDataText]}>No Entry Restricted Resi-cum-Office report data available.</Text>
      </View>
    );
  }
  ...
  return (
    <AutoSaveFormWrapper
      ...
      <ScrollView style={styles.container}>
        <Text style={[styles.title, { color: theme.colors.text }]}>Entry Restricted Residence-cum-Office Report</Text>
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          ...
          <SelectField label="Applicant Staying Status" id="applicantStayingStatus" name="applicantStayingStatus" value={report.applicantStayingStatus || ''} onValueChange={(val) => handleChange('applicantStayingStatus', val)} disabled={isReadOnly}>
            <Picker.Item label="Select..." value="" />
